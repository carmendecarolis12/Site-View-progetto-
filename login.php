...
if (isset($_POST['username'])) {
  $loginUsername=rawurlencode($_POST['username']);
 ...
 'SELECT USERNAME, PASSWORD, TIPO FROM utenti WHERE USERNAME=%s AND PASSWORD=%s',
  GetSQLValueString($loginUsername, 'text',$dbConnect),
   GetSQLValueString($password, 'text',$dbConnect)
);
  ...
  if ($loginFoundUser) {
    ...
    if (rawurlencode(isset($_SESSION['PrevUrl'])) && true) {
					...
					<article id="main">
						...
						<section class="wrapper style5">
                            ...
                            <form ACTION="<?php echo $loginFormAction; ?>" METHOD="POST" id="login">
                              ...
                              <p></p>
                                            <input type="submit" name="submit" id="submit" class="special" value="ACCEDI">
</div>
</form>
						</section>
					</article>
